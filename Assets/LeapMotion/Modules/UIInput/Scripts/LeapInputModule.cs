...
namespace Leap.Unity.InputModule
{
    ...
    public bool MovingReferenceFrame = false;
      ...
      */
		...
		public override void Process ()
		{
			...
			for (int whichPointer = 0; whichPointer < handsGameObject.Length; whichPointer++) {
				if (handsGameObject[whichPointer].activeSelf) {
				
				LeapSingleHandView leapHandView = handsGameObject [whichPointer].GetComponent<LeapSingleHandView> ();
				...
				if (perFingerPointer) {
					...
					if (cManager.Leap.Data.frame.Hands.Count - 1 < whichHand) {
				...
				if (Camera.main != null) {
					switch (cManager.Leap.Data.frame.Hands [whichHand].IsRight) {
					case true:
				...
				LeapHand hand = leapHandView.hand;
				//Raycast from shoulder through tip of the index finger to the UI
				bool TipRaycast = false;
				if (InteractionMode != InteractionCapability.Projective) {
					TipRaycast = GetLookPointerEventData (whichPointer, hand, whichFinger, ProjectionOrigin, CurrentRotation * Vector3.forward, true);
					...
					ProcessState (whichPointer, hand, whichFinger, TipRaycast);
				...
				if (((pointerState [whichPointer] == pointerStates.OffCanvas) && (InteractionMode != InteractionCapability.Tactile)) || (InteractionMode == InteractionCapability.Projective)) {
					...
					UpdatePointer (whichPointer, PointEvents [whichPointer], PointEvents [whichPointer].pointerCurrentRaycast.gameObject);
					Debug.Log(PointEvents[whichPointer].pointerCurrentRaycast.gameObject.name);
						if (!TipRaycast && (forceTactile || (!forceProjective && distanceOfTipToPointer (whichPointer, hand, whichFinger) < ProjectiveToTactileTransitionDistance))) {
				...
				if ((EnvironmentPointer) && (pointerState [whichPointer] == pointerStates.OffCanvas)) {
					Vector3 palmPosition = leapHandView.parts [(int)LeapSingleHandView.HandPart.Palm].transform.position;
					RaycastHit EnvironmentSpot;
					Vector3 direction = palmPosition - ProjectionOrigin;

					direction.Normalize ();
					//Physics.Raycast (ProjectionOrigin, palmPosition.normalized * 20, out EnvironmentSpot);

					if (Physics.Raycast (ProjectionOrigin, direction.normalized * 20, out EnvironmentSpot)) {
						Debug.DrawRay (ProjectionOrigin,EnvironmentSpot.point, Color.green);
						//Debug.Log (EnvironmentSpot.collider.name);
						Pointers [whichPointer].position = EnvironmentSpot.point ;
						Pointers [whichPointer].rotation = Quaternion.LookRotation (EnvironmentSpot.normal);

						PointerLines [whichPointer].SetPosition (0,ProjectionOrigin);
						PointerLines [whichPointer].SetPosition (1, EnvironmentSpot.point);

					} else {
						Debug.DrawRay (ProjectionOrigin, (palmPosition - ProjectionOrigin).normalized * 20, Color.green);
						Pointers [whichPointer].position = (palmPosition - ProjectionOrigin).normalized * 20 ;
						Pointers [whichPointer].rotation = Quaternion.LookRotation (Camera.main.transform.position);

						PointerLines [whichPointer].SetPosition (0,ProjectionOrigin);
						PointerLines [whichPointer].SetPosition (1,  (palmPosition - ProjectionOrigin).normalized * 20);

					}

					//Debug.DrawRay (ProjectionOrigin, (palmPosition - ProjectionOrigin).normalized * 10, Color.green);

					//Debug.Log (EnvironmentSpot.point);
					if (InnerPointer) {
						InnerPointers [whichPointer].position = EnvironmentSpot.point;
						InnerPointers [whichPointer].rotation = Quaternion.LookRotation (EnvironmentSpot.normal);
					}
					evaluatePointerSize (whichPointer);

					if (isTriggeringInteraction (whichPointer, hand, whichFinger)) {
				...
				Debug.Log (pointerState[whichPointer]);
				if ((PointEvents [whichPointer] != null)) {
					...
					if ((Hoverer != null)) {
						...
						if (comp != null) {
							...
							((ILeapWidget)comp).HoverDistance (distanceOfTipToPointer (whichPointer, hand, whichFinger));
							//}
						}
					}
					...
					if (PointEvents [whichPointer].pointerCurrentRaycast.gameObject != null && pointerState [whichPointer] != pointerStates.OffCanvas) {
						...
						if (!PrevTriggeringInteraction [whichPointer] && isTriggeringInteraction (whichPointer, hand, whichFinger)) {
					...
					if (PrevTriggeringInteraction [whichPointer] && ((!isTriggeringInteraction (whichPointer, hand, whichFinger)) || (pointerState [whichPointer] == pointerStates.OffCanvas))) {
				...
				updatePointerColor (whichPointer, hand, whichFinger);
		...
		private bool GetLookPointerEventData (int whichPointer, LeapHand hand, int whichFinger, Vector3 Origin, Vector3 Direction, bool forceTipRaycast)
		{
          ...
          IndexFingerPosition = cManager.Leap.Data.frame.Hands[whichHand].Fingers[whichFinger].TipPosition.ToVector3() * 0.1f;
          //Averages cursor position through average of extended fingers; ended up being worse than expected
          for (int i = 1; i < 4; i++) {
            float fingerExtension = Mathf.Clamp01(Vector3.Dot(cManager.Leap.Data.frame.Hands[whichHand].Fingers[i].Direction.ToVector3(), cManager.Leap.Data.frame.Hands[whichPointer].Direction.ToVector3())) / 1.5f;
            if (fingerExtension > 0f) {
              numberOfExtendedFingers += fingerExtension;
              IndexFingerPosition += cManager.Leap.Data.frame.Hands[whichHand].Fingers[i].TipPosition.ToVector3() * fingerExtension;
            }
          }
          ...
          */
					...
					float farthest = 0f;
					IndexFingerPosition = hand.Fingers [whichFinger].TipPosition.ToVector3 ();
					for (int i = 1; i < 3; i++) {
						float fingerDistance = Vector3.Distance (Camera.main.transform.position, hand.Fingers [i].TipPosition.ToVector3 ());
						float fingerExtension = Mathf.Clamp01 (Vector3.Dot (hand.Fingers [i].Direction.ToVector3 (), hand.Direction.ToVector3 ())) / 1.5f;
			...
			DebugSphereQueue.Enqueue(IndexFingerPosition);
			...
			//Debug.Log (PointEvents [whichPointer].position );
			return TipRaycast;
		}

		//Tree to decide the State of the Pointer
		private void ProcessState (int whichPointer, LeapHand hand, int whichFinger, bool forceTipRaycast)
		{
			if ((PointEvents [whichPointer].pointerCurrentRaycast.gameObject != null)) {
				if (forceTactile || (!forceProjective && distanceOfTipToPointer (whichPointer, hand, whichFinger) < ProjectiveToTactileTransitionDistance)) {
					if (isTriggeringInteraction (whichPointer, hand, whichFinger)) {
		...
		void evaluatePointerSize (int whichPointer)
		{
			...
			if (!perFingerPointer && !getTouchingMode (whichPointer)) {
				if (whichPointer == 0) {
					Pointerscale *= PointerPinchScale.Evaluate (cManager.Leap.Data.frame.Hands [0].PinchDistance);
				} else if (whichPointer == 1) {
					Pointerscale *= PointerPinchScale.Evaluate (cManager.Leap.Data.frame.Hands [1].PinchDistance);
				}
			}
		...
		public bool isTriggeringInteraction (int whichPointer,LeapHand hand, int whichFinger)
		{

			if (InteractionMode != InteractionCapability.Projective) {
				if (getTouchingMode (whichPointer)) {
					return (distanceOfTipToPointer (whichPointer, hand, whichFinger) < 0f);
				}
			}
			/*
      if (InteractionMode != InteractionCapability.Tactile) {
        if ((cManager.Leap.Data.frame.Hands[whichHand].IsRight) && (RightHandDetector != null && RightHandDetector.IsPinching) || (RightHandDetector == null && cManager.Leap.Data.frame.Hands[whichHand].PinchDistance < PinchingThreshold)) {
          return true;
        } else if ((cManager.Leap.Data.frame.Hands[whichHand].IsLeft) && (LeftHandDetector != null && LeftHandDetector.IsPinching) || (LeftHandDetector == null && cManager.Leap.Data.frame.Hands[whichHand].PinchDistance < PinchingThreshold)) {
          return true;
        }
      } */

			//Disabling Pinching during touch interactions; maybe still desirable?
			//return cManager.Leap.Data.frame.Hands[whichPointer].PinchDistance < PinchingThreshold;

			return false;
		}

		/** The z position of the index finger tip to the Pointer. */
		public float distanceOfTipToPointer (int whichPointer, LeapHand hand, int whichFinger)
		{
			//Get Base of Index Finger Position
			Vector3 TipPosition = hand.Fingers [whichFinger].TipPosition.ToVector3 ();
		...
		{
			//Get Base of Index Finger Position
			Vector3 TipPosition = cManager.Leap.Data.frame.Hands [whichHand].Fingers [whichFinger].TipPosition.ToVector3 ();
		...
		void updatePointerColor (int whichPointer, LeapHand hand, int whichFinger)
		{
			float TransitionAmount = Mathf.Clamp01 (Mathf.Abs ((distanceOfTipToPointer (whichPointer, hand, whichFinger) - ProjectiveToTactileTransitionDistance)) / 0.05f);
     ...
     */
		...
		{
			return handsGameObject[0].activeSelf && base.ShouldActivateModule ();
		}
	}
}
